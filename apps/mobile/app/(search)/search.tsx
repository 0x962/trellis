...
import { pushRecent, readRecents, recentSearchesKey } from "../../src/features/search/recentSearches";
...
export default function SearchScreen() {
	...
	useEffect(() => {
		if (search.data !== undefined && enabled) pushRecent(store, query);
	}, [enabled, query, search.data]);

	const openTicket = (identifier: string) => router.push(`/ticket/${identifier}`);
	const submit = () => {
			...
			{!enabled ? (
				recents.length === 0 ? (
					...
					<View style={styles.recents}>
						...
						{recents.map((recent) => (
							<Pressable key={recent} onPress={() => setText(recent)} style={styles.recent}>
