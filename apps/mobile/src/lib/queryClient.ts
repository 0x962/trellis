...
// request settles at once: the unreachable-server screen and the toast carry
// their own Retry, so no query retries on its own.
export const queryClient = new QueryClient({
	defaultOptions: { queries: { staleTime: Number.POSITIVE_INFINITY, gcTime: 86_400_000, retry: false } },
});
