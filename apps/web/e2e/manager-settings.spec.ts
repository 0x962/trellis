...
test("the manager page does not show the manager queue", async ({ page }) => {
	await post<Project>("/projects", { key: "MQH", name: "Manager queue hidden" });
	...
	await expect(page.getByRole("heading", { name: "Manager queue", exact: true })).toHaveCount(0);
	await expect(page.getByText("No manager deliveries are recorded.", { exact: true })).toHaveCount(0);
});
