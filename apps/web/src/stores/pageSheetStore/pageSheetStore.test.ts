...
beforeEach(() => usePageSheetStore.setState({ ticket: null, pr: null, stats: null }));

test("the stack starts empty", () => {
	expect(state()).toEqual({ ticket: null, pr: null, stats: null });
});
...
test("a pull request opens over the ticket and leaves it open", () => {
	...
	expect(state()).toEqual({ ticket: "TRL-42", pr: "https://github.com/o/r/pull/7", stats: null });
});

test("a pull request opens with no ticket under it", () => {
	pageSheetActions.openPullRequest("https://github.com/o/r/pull/7");

	expect(state()).toEqual({ ticket: null, pr: "https://github.com/o/r/pull/7", stats: null });
});
...
test("closing the pull request leaves the ticket open", () => {
	...
	expect(state()).toEqual({ ticket: "TRL-42", pr: null, stats: null });
});
...
test("another ticket closes the pull request of the ticket before it", () => {
	...
	expect(state()).toEqual({ ticket: "TRL-43", pr: null, stats: null });
});
...
test("statistics opens as the only sheet", () => {
	...
	expect(state()).toEqual({ ticket: null, pr: null, stats: "TRL/runtime" });
});
