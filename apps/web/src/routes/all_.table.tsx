...
function AllPage() {
	...
	const setSearch = (next: Partial<View>) => navigate({ to: "/all/table", search: stripDefaults(next) });

	const switchView = (next: ListView) => {
		if (next === "board") void router.navigate({ href: "/all" });
		else void navigate({ to: "/all/table", search });
	};
