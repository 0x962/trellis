...
export function ProcessTable({ processes }: { processes: readonly SystemProcess[] }) {
	...
	return (
		<section aria-labelledby="processes-title" className="flex flex-col gap-3">
					...
					<table className="w-full table-fixed border-collapse text-sm">
						<thead>
							<tr className="border-b border-border text-left text-xs text-fg-faint">
								...
								<th scope="col" className="h-8 w-20 pr-3 text-right font-medium max-sm:hidden">
									PID
								</th>
						...
						<tbody>
							{visible.slice(0, shown).map((process) => (
								<tr key={process.pid} className="h-9 border-b border-border hover:bg-band">
									<td className="truncate pr-3 font-mono text-xs text-fg" title={process.command}>
										{process.command}
									</td>
									<td className="pr-3 text-right text-fg-muted tabular max-sm:hidden">{process.pid}</td>
									...
									<td className="pr-3 text-right text-fg tabular" title={formatPercent(process.memoryPercent)}>
