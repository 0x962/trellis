...
describe("UsageGroups", () => {
	test("prints one row per slice with its share, and a click selects the row", async () => {
		...
		expect(screen.getByRole("link", { name: "Add the usage page" }).getAttribute("href")).toBe("/t/CDE-7");
	...
	test("a selected row is pressed and a second click clears it", async () => {
		const user = userEvent.setup();
		const onSelectRow = mock();
		mount(
			<UsageGroups
