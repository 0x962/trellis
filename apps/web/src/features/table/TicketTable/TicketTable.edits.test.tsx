...
describe("features/table/TicketTable: inline edits", () => {
	...
	test("applies an inline status change optimistically", async () => {
		...
		const slow = held();
		const { server } = renderApp({ path: "/p/CDE/web?status=todo,in-progress", actor: "navid", server: slow.server });
		await findGrid();
	...
	test("applies an inline priority change optimistically and re-sorts the group", async () => {
		const user = userEvent.setup();
		const slow = held();
		renderApp({ path: "/p/CDE/web?status=todo", actor: "navid", server: slow.server });
		await findGrid();
	...
	test("moves the row between project groups optimistically", async () => {
		const user = userEvent.setup();
		const slow = held();
		renderApp({ path: "/p/CDE?status=in-progress&group=project", actor: "navid", server: slow.server });
		await findGrid();
		await waitFor(() => rowOf("CDE-44"));
	...
	test("never edits the title inline in the table", async () => {
		const { router } = renderApp({ path: "/p/CDE?status=in-progress", actor: "navid" });
		await findGrid();
		await waitFor(() => rowOf("CDE-44"));
