...
// group. The last group runs to the end of the body.
export type WaveBox = { top: number; height: number };

// The box of each group header, by the index of that header line in the
// list. The header of a group stays at the top of the scroll container
// while its box passes, so the header of the next group pushes it out.
//
...
export const waveBoxes = (
	...
	headerIndexes.forEach((index, order) => {
		const top = lines[index]?.start;
		if (top === undefined) return;
		const nextIndex = headerIndexes[order + 1];
		const end = (nextIndex === undefined ? undefined : lines[nextIndex]?.start) ?? totalSize;
		boxes.set(index, { top, height: Math.max(end - top, 0) });
	});
	return boxes;
};
