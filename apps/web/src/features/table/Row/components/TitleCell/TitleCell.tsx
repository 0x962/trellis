...
// truncates, so the row keeps its height. A done or canceled ticket shows
// its title alone, so finished work stays quiet, and its caret for the
// hidden pull request and agent lines sits at the right end of the cell.
export function TitleCell({ ticket, disclosure, onToggleDisclosure }: TitleCellProps) {
	...
	const closed = ticket.status.category === "done" || ticket.status.category === "canceled";
	return (
