...
const lineOf = (words: string, fields: Partial<TicketAgentLine> = {}): TicketAgentLine => ({
	words,
...
describe("AgentLine", () => {
	test("prints the words of a message in the muted color and draws no dot", () => {
		const html = renderToStaticMarkup(
			<AgentLine line={lineOf("crisp-fjord: I rebased.")} top={0} last depth={1} render={render} />,
		);
	...
	test("draws the dot and the warning color when the run asks", () => {
		const html = renderToStaticMarkup(
			<AgentLine
				line={lineOf("crisp-fjord asks: Which cap?", { asks: true })}
				top={0}
				last
				depth={1}
				render={render}
	...
	test("takes at least the height the virtual list reserves, at the offset it names", () => {
		const html = renderToStaticMarkup(
			<AgentLine line={lineOf("crisp-fjord: I rebased.")} top={288} last depth={1} render={render} />,
		);
	...
	test("wraps a long message and never truncates it", () => {
		...
		const html = renderToStaticMarkup(<AgentLine line={lineOf(words)} top={0} last depth={1} render={render} />);
	...
	test("holds the markdown to the size of the line, so a heading cannot shout", () => {
		const html = renderToStaticMarkup(
			<AgentLine line={lineOf("# crisp-fjord pushed it")} top={0} last depth={1} render={render} />,
		);
	...
	test("hangs from the last pull request of the ticket at the deeper tree level", () => {
		const html = renderToStaticMarkup(
			<AgentLine line={lineOf("crisp-fjord: I rebased.")} top={0} last depth={2} render={render} />,
		);
	...
	test("runs the rule of the ticket through the line when a merged pull request follows it", () => {
		const html = renderToStaticMarkup(
			<AgentLine line={lineOf("crisp-fjord: I rebased.")} top={0} last={false} depth={2} render={render} />,
		);
	...
	test("drops the shimmer and renders markdown when the run ends its turn", () => {
		const html = renderToStaticMarkup(
			<AgentLine line={lineOf("crisp-fjord: The branch is ready.")} top={0} last depth={1} render={render} />,
		);
	...
	test("carries the index the virtualizer reads when it measures the line", () => {
		const html = renderToStaticMarkup(
			<AgentLine line={lineOf("crisp-fjord: I rebased.")} top={0} last depth={1} index={7} render={render} />,
		);
