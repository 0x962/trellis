...
const ticket = (id: string, category: string) => ({ id, status: { category } }) as TicketSummary;
...
test("ranks by what the person does next", () => {
	...
	expect(rank(ticket("waits", "review"))).toBe(0);
	expect(rank(ticket("reviewed", "review"))).toBe(0);
