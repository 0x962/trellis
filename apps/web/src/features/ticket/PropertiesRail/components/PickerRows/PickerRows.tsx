...
import type { Priority, ProjectSummary, Status, Ticket } from "@trellis/api";
...
export function PickerRows({ ticket }: PickerRowsProps) {
	...
	const pickProject = async (project: ProjectSummary) => {
		...
		<>
			...
			<Row label="Project">
				<ProjectPicker
					...
					onPick={(project) => void pickProject(project)}
					open={open === "project"}
					onOpenChange={(next) => {
			...
			<Row label="Parent">
				<TicketPicker
					...
					rootKey={ticket.project.key}
					exclude={ticket.identifier}
					value={ticket.parent?.identifier ?? null}
					onPick={(parent) => void pickParent(parent)}
					open={open === "parent"}
