...
describe("evidenceOwedText", () => {
	...
	test("names the backend floor when no file renders a route", () => {
		...
		expect(text).toBe("backend: summary · working call · failing call");
	});
	...
	test("names both floors when the files render a route and change the server", () => {
		...
		expect(text).toBe(
			"mixed: summary · after image · before image · capture record · console list · working call · failing call",
		);
	});
