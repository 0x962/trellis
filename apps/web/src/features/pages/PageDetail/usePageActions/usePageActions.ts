...
export const pageActionError = (error: Error) => {
...
export const usePageActions = (page: PageRecord, offline: boolean, archived: boolean) => {
	...
	const mutation = useMutation({
		mutationFn: async (action: "pin" | "delete" | "restore" | "pull") => {
			if (offline) throw new Error("The server is offline.");
			if (archived && action !== "pull") throw new Error("The project is archived.");
		...
		onSuccess: async (_data, action) => {
			await invalidate();
			toast.success(action === "pull" ? "Page download started" : "Page updated");
		},
		onError: async (error) => {
