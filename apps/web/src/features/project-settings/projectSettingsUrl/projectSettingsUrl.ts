...
// the empty string.
export type ProjectSettingsSectionId = "" | "workflow" | "notes" | "archive";

const sectionIds: readonly ProjectSettingsSectionId[] = ["", "workflow", "notes", "archive"];
...
export const projectSettingsSection = (view: "settings" | "notes", hash: string): ProjectSettingsSectionId =>
	view === "notes"
		? "notes"
		: hash === "template" || hash === "statuses" || hash === "labels"
			? "workflow"
			: isSectionId(hash)
				? hash
				: "";
