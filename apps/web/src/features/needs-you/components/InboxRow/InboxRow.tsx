	...
	return (
		...
		>
			<td className="flex min-w-0 flex-1 flex-col p-0">
				...
				<span className="flex h-9 shrink-0 items-center gap-3 px-5">
					...
					{showStatus && (
						<span className="flex w-36 shrink-0 items-center gap-1.5 text-sm text-fg-faint">
							<StatusIcon category={status.category} reviewer={status.reviewer ?? undefined} />
							<span className="truncate">{status.name}</span>
						</span>
					)}
					<span className={cx("flex w-16 shrink-0 items-center gap-1.5", covered)}>
						{pr !== null && (
					...
					<span className={cx("flex w-10 shrink-0 justify-center", covered)}>
