...
test("the key holds all four values", () => {
	expect(choiceKey(choice({ effort: "high", accountId: WORK }))).toBe(`claude||high|${WORK}`);
	expect(choiceKey(choice())).not.toBe(choiceKey(choice({ effort: "high" })));
});
...
test("the detail line of Muse holds the account alone", () => {
	expect(choiceDetail(choice({ preset: "muse" }), [])).toBe("Default account");
	expect(choiceDetail(choice({ effort: "low", accountId: WORK }), [account()])).toBe("Low · Work");
});

test("the title names the harness, the model, the effort and the account", () => {
	expect(choiceTitle(choice(), [])).toBe("Claude · Claude Opus 5 · Harness default · Default account");
});
...
test("the dialog draft drops the values the machine no longer holds", () => {
	expect(draftFrom(choice({ model: "openai/gpt-5.6-sol", effort: "high", accountId: WORK }), [])).toEqual(
		choice({ effort: "high" }),
	);
	expect(draftFrom(choice({ effort: "ultra" }), [account()])).toEqual(choice());
});

test("the dialog draft keeps an account while the list loads", () => {
	expect(draftFrom(choice({ accountId: WORK }), undefined).accountId).toBe(WORK);
	expect(draftFrom(choice({ accountId: WORK }), [account()]).accountId).toBe(WORK);
	expect(draftFrom(choice({ accountId: WORK }), []).accountId).toBeNull();
});
