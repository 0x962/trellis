...
test("manual sessions precede copilots and stay separate from ticket agents", () => {
	const runs = [
		run("ticket"),
		run("copilot", { kind: "manager", ticketId: null }),
		run("manual", { kind: "session", ticketId: null }),
	];
	const groups = sessionGroups(runs, { search: "", history: false });
	expect(groups.sessions.map((item) => item.id)).toEqual(["manual", "copilot"]);
