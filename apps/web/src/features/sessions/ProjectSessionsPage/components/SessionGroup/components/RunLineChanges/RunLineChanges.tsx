...
// present, so a title never moves when a count arrives. The column shows an
// ellipsis while the counts load, and stays blank when the workspace has no
// changes, is gone, or Git could not read it. A row in a collapsed group
// reads nothing.
export function RunLineChanges({ run, enabled }: { run: WorkspaceSummaryRun; enabled: boolean }) {
	...
	if (summary.state === "ready" && (summary.additions !== 0 || summary.deletions !== 0))
