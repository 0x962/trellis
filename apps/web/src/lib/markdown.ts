...
const marked = new Marked({ gfm: true, breaks: false, extensions: [ticketIdExtension], renderer });

// Renders markdown to HTML for a read-only view. Descriptions and comments
// come from agents and from curl, so nothing that runs survives: no script,
// no event handler, no javascript: link.
export const renderMarkdown = (markdown: string): string => sanitizeHtml(marked.parse(markdown, { async: false }));
