...
// GET /api/attachments/{id}/file serves the stored bytes. The ETag is the
// hash, so a browser that holds the file never downloads it again. The
// sandbox policy and nosniff keep an uploaded file from running as script
// on the app origin. A type outside the inline allowlist downloads.
