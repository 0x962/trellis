...
// of the epic, so the composite foreign key keeps the wave and its epic
// in one root. The unique (epic_id, slug) pair gives the wave its
// `KEY/epic-slug/wave-slug` ref. The wave keeps no state column:
...
export const waves = pgTable(
	...
	(t) => [
		unique("waves_id_epic_id_unique").on(t.id, t.epicId),
