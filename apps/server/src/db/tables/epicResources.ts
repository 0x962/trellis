...
export const epicResources = pgTable(
	...
	(t) => [
		...
		check(
			...
			sql`(${t.kind} = 'doc' AND ${t.body} IS NOT NULL AND ${t.url} IS NULL AND ${t.blobSha256} IS NULL)
				OR (${t.kind} = 'link' AND ${t.body} IS NULL AND ${t.url} IS NOT NULL AND ${t.blobSha256} IS NULL)
				OR (${t.kind} IN ('image', 'file') AND ${t.body} IS NULL AND ${t.url} IS NULL AND ${t.blobSha256} IS NOT NULL)`,
		),
		index("epic_resources_epic_id_created_at_idx").on(t.epicId, t.createdAt),
		index("epic_resources_blob_sha256_idx").on(t.blobSha256),
