...
// waves of the tests, and TST/other is a second epic of the same root.
...
test("create appends the wave, derives the slug, and suffixes a taken derived slug", async () => {
...
test("a given slug that another wave of the epic holds is DUPLICATE", async () => {
	...
	await expect(run((tx) => update(ctx, tx, { wave: "TST/plan/phase-2-2", slug: "phase-2" }))).rejects.toMatchObject({
		code: "DUPLICATE",
		data: { field: "slug" },
	});
	const other = await run((tx) => create(ctx, tx, { epic: "TST/other", name: "Phase 2" }));
	expect(other.ref).toBe("TST/other/phase-2");
	const renamed = await run((tx) => update(ctx, tx, { wave: "tst/plan/PHASE-2-2", name: "Phase 3", slug: "phase-3" }));
...
test("reorder takes the full list of the epic and writes the positions from 0", async () => {
	...
	await expect(
		run((tx) => reorder(ctx, tx, { epic: "TST/plan", waves: ["TST/plan/phase-3", "TST/plan/phase-2"] })),
	).rejects.toMatchObject({ code: "WAVE_OUTSIDE_EPIC" });
	await expect(
		run((tx) =>
			reorder(ctx, tx, {
				epic: "TST/plan",
				waves: ["TST/plan/phase-3", "TST/plan/phase-2", "TST/other/phase-2"],
			}),
		),
	).rejects.toMatchObject({ code: "WAVE_OUTSIDE_EPIC" });
	await expect(
		run((tx) =>
			reorder(ctx, tx, {
				epic: "TST/plan",
				waves: ["TST/plan/phase-3", "TST/plan/phase-3", "TST/plan/phase-2"],
			}),
		),
	).rejects.toMatchObject({ code: "WAVE_OUTSIDE_EPIC" });
	const ordered = await run((tx) =>
		reorder(ctx, tx, {
			epic: "TST/plan",
			waves: ["TST/plan/phase-3", "TST/plan/phase-1-run-state", "TST/plan/phase-2"],
		}),
	);
	expect(ordered.map((wave) => [wave.slug, wave.position])).toEqual([
		["phase-3", 0],
		["phase-1-run-state", 1],
		["phase-2", 2],
	]);
	const epic = await run((tx) => getEpic(ctx, tx, { epic: "TST/plan" }));
	expect(epic.waves.map((wave) => wave.slug)).toEqual(["phase-3", "phase-1-run-state", "phase-2"]);
});
...
test("delete needs force for an agent, then detaches every ticket with activity and events", async () => {
	...
	await expect(run((tx) => remove(asAgent, tx, { wave: "TST/plan/phase-2" }))).rejects.toMatchObject({
	...
	const activity = await db.execute(
		sql`SELECT from_value, to_value, count(*)::int AS n FROM activity
			WHERE field = 'wave' AND meta->>'fromId' = ${removed.id} AND created_at = '2026-09-18T10:07:00.000Z'
			GROUP BY from_value, to_value`,
	);
	expect(activity.rows).toEqual([{ from_value: "TST/plan/phase-2", to_value: null, n: 2 }]);
	expect(
		events.map((event) =>
			event.type === "ticket.updated" ? [event.summary.identifier, event.fields, event.summary.wave] : event,
		),
	).toEqual([
		["TST-1", ["wave"], null],
		["TST-2", ["wave"], null],
	...
	await expect(run((tx) => update(asAgent, tx, { wave: "TST/plan/phase-2", name: "Gone" }))).rejects.toMatchObject({
		code: "NOT_FOUND",
		data: { kind: "wave", ref: "TST/plan/phase-2" },
	});
});
