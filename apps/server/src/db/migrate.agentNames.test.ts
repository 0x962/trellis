...
import { seedProject, seedTicket } from "../../test/fixtures";
...
// name, so no row is left without one.
...
let temp: string;
beforeAll(() => {
	temp = mkdtempSync(join(import.meta.dir, "../../.cache/migrate-names-"));
});
afterAll(() => rmSync(temp, { recursive: true, force: true }));

// A copy of the migration directory whose journal stops before 0012, so a
// migration run of it leaves the schema as it was before this change.
const beforeNames = () => {
	const dir = join(temp, "before");
	...
	journal.entries = journal.entries.filter((entry) => entry.tag !== NAMED);
...
test("server builder: migration 0012 names every session that the database already holds", async () => {
	...
	expect(await migrate(db, beforeNames())).toBeGreaterThan(0);
	...
	expect(await migrate(db, drizzleDir)).toBe(1);
