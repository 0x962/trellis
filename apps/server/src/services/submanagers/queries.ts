...
export const usage = async (
	tx: Tx,
	input: { projectId: string; runId: string; excludeRunId?: string } & CapacityObservation,
) => {
	const [counts] = await rows<{ activeWorkers: number; childCapacity: number }>(
		...
		(SELECT count(*)::int FROM agent_runs WHERE project_id IN (${managerScope(input.projectId, true)})
		 AND kind<>'manager' AND runtime='native' AND closed_at IS NULL AND id IS DISTINCT FROM ${input.excludeRunId ?? null}
 AND ${occupiesSlot(sql`terminal_id`, input)}) AS "activeWorkers",
