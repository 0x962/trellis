...
import { count, hoursAgo, seedProject, seedTicket } from "../../test/fixtures";
...
import { gcAttachmentBlobs, get, remove, upload } from "./attachments.ts";
...
describe("attachments.remove", () => {
	...
	test("a delete emits attachment.deleted and moves the ticket", async () => {
		...
		const expected: TrellisEvent[] = [{ type: "attachment.deleted", id: uploaded.attachment.id, ticketId: ticket }];
