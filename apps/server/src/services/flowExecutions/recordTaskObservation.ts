...
export async function recordTaskObservation(
	...
	const [run] = await rows<{ session_id: string | null; error: string | null }>(
		tx,
		sql`SELECT session_id,error FROM agent_runs WHERE id=${task.run_id} AND terminal_id=${input.attemptId} FOR UPDATE`,
	);
	...
	if (!run || (run.session_id !== null && run.session_id !== snapshot.sessionId))
		event = unknown("The result does not belong to the current flow attempt");
	else {
		// A provider can report its session after startNative saves the process.
		// The task and terminal checks above bind this observation to its attempt.
		if (run.session_id === null && snapshot.sessionId !== null)
			await tx.execute(
				sql`UPDATE agent_runs SET session_id=${snapshot.sessionId} WHERE id=${task.run_id} AND terminal_id=${input.attemptId}`,
			);
		if (snapshot.state === "failed")
			event = {
				...
				error: describeTaskFailure(
					...
					execution.state,
					step,
					run.error ?? snapshot.error ?? "The flow worker failed",
				),
			};
		else if (snapshot.state === "unknown")
			event = unknown(run.error ?? snapshot.error ?? "The flow worker needs attention");
