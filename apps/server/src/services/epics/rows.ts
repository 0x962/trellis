...
// The alias `c` counts the tickets of the epic by status category in one
// pass over the tickets that point at it. An agent actor is `agent:<run id>`,
// and the run name is its display name.
export const epicSelect = sql`SELECT e.id, e.project_id, e.root_id, root.key AS root_key, e.slug, e.name, e.description,
	...
	CROSS JOIN LATERAL (
		...
		FROM tickets t JOIN statuses s ON s.id = t.status_id WHERE t.epic_id = e.id
	) c`;
