...
export async function deliveryRows(tx: Tx, id: string) {
	return rows<ReviewDelivery>(
		tx,
		sql`SELECT id, review_id AS "reviewId", ticket_id AS "ticketId", run_id AS "runId", state, error, to_char(read_at AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"') AS "readAt" FROM review_deliveries WHERE review_id = ${id} ORDER BY id`,
	);
}
