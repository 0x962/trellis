...
const fields =
	"title,state,isDraft,author,headRepository,headRefName,baseRefName,headRefOid,baseRefOid,additions,deletions,changedFiles,mergeable,autoMergeRequest,body,comments,reviews,statusCheckRollup,labels,commits";
...
export async function prepareFile(
	...
	const ref = parseRef(input.pr);
	const meta = doc.meta as { comparisonBaseSha: string; headRepository?: { nameWithOwner: string } };
	const repo =
		input.side === "new" && meta.headRepository ? meta.headRepository.nameWithOwner : `${ref.owner}/${ref.repo}`;
	const sha = input.side === "old" ? meta.comparisonBaseSha : doc.headSha;
	return {
		content: await gh(ctx, [
			"api",
			`repos/${repo}/contents/${input.path.split("/").map(encodeURIComponent).join("/")}?ref=${sha}`,
			"-H",
