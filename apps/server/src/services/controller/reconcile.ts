...
export const reconcile = async (ctx: ControllerCtx, tx: Tx, input: ControllerInput) => {
	...
	for (const delivery of unknown) {
		if (
			!input.sessions
				.find((session) => session.id === delivery.terminalId)
				?.acknowledgedMessageIds.includes(dispatchMessageId(delivery))
		)
			continue;
