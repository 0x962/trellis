...
import { expectError, query, serviceHarness, ticketRow, traceRows } from "../../test/helpers/services.ts";
...
describe("tickets.delete", () => {
	...
	test("delete with an unknown ref throws NOT_FOUND", async () => {
		...
		const data = await expectError(remove({ ticket: "CDE-999" }), "NOT_FOUND");
	...
	test("delete in an archived project throws PROJECT_ARCHIVED", async () => {
		...
		await expectError(remove({ ticket: id }), "PROJECT_ARCHIVED");
