...
export const outcome = async (ctx: ServiceCtx, tx: Tx, rawInput: unknown): Promise<Ticket> => {
	...
	if (current!.outcome === input.text) return ticketGet(tx, target.id);

	const batchId = ulid();
	await tx.execute(sql`UPDATE tickets SET outcome = ${input.text}, version = version + 1, updated_at = ${ctx.now}
		WHERE id = ${target.id}`);
	await record(ctx, tx, {
		...
		batchId,
		changes: [{ field: "outcome", from: current!.outcome, to: input.text }],
	});
