...
const toFilter = async (ctx: Ctx, tx: Tx, query: Query) => {
...
export const list = async (ctx: Ctx, tx: Tx, rawInput: unknown) => {
	...
	try {
		...
		if (error instanceof InvalidCursorError) throw contractError("INVALID_CURSOR", undefined);
		throw error;
	}
};

export const boardOf = async (ctx: Ctx, tx: Tx, rawInput: unknown) => {
...
export const countsOf = async (ctx: Ctx, tx: Tx, rawInput: unknown) => {
	const query = CountsQuerySchema.parse(rawInput);
	const { filter, projectId } = await toFilter(ctx, tx, query);
	return counts(tx, { ...filter, statusIds: await columnStatusIds(tx, projectId) });
};

export const get = async (ctx: Ctx, tx: Tx, rawInput: unknown): Promise<Ticket> => {
