...
export const reserve = async (
	...
	const [run] =
		...
		VALUES (${ulid()}, ${name}, ${JSON.stringify(config.harness)}::jsonb, ${kind}, ${options?.flow?.instruction ?? (kind === "manager" ? config.instruction : "")}, ${project.id}, ${projectPath}, ${ticket?.id ?? null}, ${ticket?.identifier ?? null}, 'native', NULL, ${sessionId}, ${ctx.now}, ${ctx.now})
