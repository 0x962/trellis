...
test("a poll that finds a new head commit clears the moment the wait started", async () => {
	const { id } = await readyPullRequest("Poll after a push", 113);
	await poll(113, "head113");

	const kept = await readyAtOf(id);
	await poll(113, "pushed113");
...
test("a merge leaves the moment the wait started", async () => {
	const { id } = await readyPullRequest("Merge after the ask", 114);

	await poll(114, "head114", "merged");
