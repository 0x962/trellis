...
// Blocks model, ticket, commit, and branch terms unless the user made the term the subject.
...
export const checkGeneratedName = (initialPrompt: string, candidateName: string, protectedTerms: string[] = []) => {
	const name = SessionGeneratedNameSchema.parse(candidateName.trim());
	for (const match of name.matchAll(forbiddenNameTerm))
		if (!initialPrompt.toLowerCase().includes(match[0].toLowerCase()))
			throw new Error(`The session name contains a term that the user did not make the subject: ${match[0]}`);
	for (const term of protectedTerms)
		if (
			term !== "" &&
			name.toLowerCase().includes(term.toLowerCase()) &&
			!initialPrompt.toLowerCase().includes(term.toLowerCase())
		)
