...
import { activityRows, at, expectError, serviceHarness, ticketRow, traceRows } from "../../test/helpers/services.ts";
...
describe("agent policy", () => {
	test("an agent move to a done status throws AGENT_CANNOT_COMPLETE", async () => {
		...
		const data = await expectError(move(claude, { ticket: id, status: "done" }), "AGENT_CANNOT_COMPLETE");
	...
	test("force lets an agent move a ticket to a done status", async () => {
		...
		const statusRow = (await activityRows(h.db, id)).find((activity) => activity.field === "status");
