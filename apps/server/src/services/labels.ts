...
// The root project of a tree owns the labels of the tree. A call names any
// project of the tree, and the service reads and writes the labels of its
// root. Every write records one project activity row on the root and tells
// the clients of the root to read the labels again.
...
export const list = async (ctx: ServiceCtx, tx: Tx, input: ListInput): Promise<LabelListOutput> => {
	...
	const groups = await groupsOfRoot(tx, project.rootId);
	const labels = await labelsOfRoot(tx, project.rootId);
...
export const create = async (ctx: ServiceCtx, tx: Tx, input: CreateInput): Promise<Label> => {
	...
	const group = input.group === undefined ? null : await resolveLabelGroup(ctx, tx, { rootId, ref: input.group });
	if (group === null) await assertTopLevelNameFree(tx, rootId, input.name, {});
	else await assertGroupNameFree(tx, group.id, input.name, null);
	const id = ulid();
	const color = input.color ?? (await autoColor(tx, rootId));
	await tx.execute(
		sql`INSERT INTO labels (id, project_id, group_id, name, color, description, created_at, updated_at)
			VALUES (${id}, ${rootId}, ${group?.id ?? null}, ${input.name}, ${color}, ${input.description ?? ""},
				${ctx.now}, ${ctx.now})`,
	);
	const created = await labelById(tx, rootId, id);
	await projectActivity(ctx, tx, rootId, "label.created", [
...
export const update = async (ctx: ServiceCtx, tx: Tx, input: UpdateInput): Promise<Label> => {
	const project = await resolveMutableProject(ctx, tx, input.project);
	const rootId = project.rootId;
	const label = await labelById(tx, rootId, input.label);
	const group =
		input.group === undefined || input.group === null
			? null
			: await resolveLabelGroup(ctx, tx, { rootId, ref: input.group });
	...
	if (movesGroup || name !== label.name) {
		if (groupId === null) await assertTopLevelNameFree(tx, rootId, name, { labelId: label.id });
	...
	emitLabelsChanged(ctx, rootId);
	return toLabel(await labelById(tx, rootId, label.id));
};
...
// `tickets.version` and `tickets.updated_at` alone; the clients of the root
...
const remove = async (ctx: ServiceCtx, tx: Tx, input: DeleteInput): Promise<DeleteOutput> => {
	...
	const label = await labelById(tx, project.rootId, input.label);
	await tx.execute(sql`DELETE FROM labels WHERE id = ${label.id}`);
	await projectActivity(ctx, tx, project.rootId, "label.deleted", [
		{
			field: null,
	...
	emitLabelsChanged(ctx, project.rootId);
