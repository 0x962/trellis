import type { AgentRegisterInput, AgentSession, AgentSessionsInput } from "@trellis/api";
import { sql } from "drizzle-orm";
import { managerTitle, reviewerTitle } from "../agents/names.ts";
...
export const register = async (ctx: ServiceCtx, tx: Tx, input: AgentRegisterInput): Promise<AgentSession> => {
	...
	const title =
		ticket === null
			? managerTitle(pathOf(ctx.cache, project.id))
			: input.role === "builder"
				? ticket.identifier
				: reviewerTitle(ticket.identifier);
