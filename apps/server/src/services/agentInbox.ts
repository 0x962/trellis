...
const commentsOf = async (tx: Tx, events: Activity[]) => {
	...
	const found = await rows<RawComment>(
		tx,
		sql`WITH changed AS (SELECT id, parent_id FROM comments WHERE id = ANY(${textArray([...new Set(ids)])}))
			SELECT c.id, c.ticket_id, c.parent_id, ${iso(sql`c.resolved_at`)} AS resolved_at, c.body, c.actor_name, c.actor_kind,
				${iso(sql`c.created_at`)} AS created_at, ${iso(sql`c.updated_at`)} AS updated_at
			FROM comments c WHERE c.id IN (SELECT id FROM changed UNION SELECT parent_id FROM changed WHERE parent_id IS NOT NULL) ORDER BY c.created_at, c.id`,
	);
...
export const inbox = async (ctx: ServiceCtx, tx: Tx, rawInput: unknown): Promise<AgentInboxOutput> => {
	...
	const manager = agentActorName({ role: "manager", project: pathOf(ctx.cache, project.id) });
