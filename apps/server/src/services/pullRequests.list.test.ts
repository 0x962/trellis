...
import { claude, linkPr, navid, seedPr, seedProject, seedTicket } from "../../test/fixtures";
...
describe("pullRequests.list", () => {
	test("list returns the linked pull requests of a ticket", async () => {
		...
		await linkPr(h.db, ticket, older, navid, "manual");
		...
		expect(items[1]).toMatchObject({ number: 11, source: "manual", linkedBy: { name: navid.name, kind: navid.kind } });
	});
});
