...
// indented under it, as the brief prints a comment. Every note of
// `projectPath` and of the projects above it is in the list, so the item
// names the project that owns it.
export const notesLines = (notes: Note[], projectPath: string): string[] => {
	...
	for (const note of notes.slice(0, NOTES_PROMPT_LIMIT)) {
		...
		lines.push(
			`- ${note.title} (${note.projectPath}, ${note.actor.displayName ?? note.actor.name}, updated ${note.updatedAt}${expiry}):`,
		);
		for (const line of note.body.split("\n")) lines.push(`  ${line}`);
	}
	if (notes.length > NOTES_PROMPT_LIMIT)
		lines.push(`- ${notes.length - NOTES_PROMPT_LIMIT} more: trellis notes list ${projectPath}`);
	return lines;
};
