...
// The version that `requestId` already created. A caller that sends one
// publication twice reads the same version both times and adds no second one.
const versionOfRequest = async (tx: Tx, requestId: string) => {
	const [row] = await rows<{ page_id: string; number: number }>(
		tx,
		sql`SELECT page_id, number FROM page_versions WHERE request_id = ${requestId}`,
	);
	return row;
};
...
// The agent run that published this version. An agent acts under the
// identifier of its run, and a name that no run holds records no run.
...
const result = async (ctx: ServiceCtx, tx: Tx, pageId: string, number: number): Promise<PagePublishOutput> => ({
...
export const publish = async (ctx: ServiceCtx, tx: Tx, rawInput: unknown): Promise<PagePublishOutput> => {
	...
	if (repeated !== undefined) return result(ctx, tx, repeated.page_id, repeated.number);
	...
	if (assetBytes > PAGE_VERSION_ASSET_BYTES_MAX)
		throw invalidInput("assets", `The assets of one version hold ${PAGE_VERSION_ASSET_BYTES_MAX} bytes at most.`);
	...
	for (const asset of input.assets) {
		...
		await tx.execute(sql`INSERT INTO page_assets (page_id, version, path, sha256, size, mime)
			VALUES (${pageId}, ${number}, ${asset.path}, ${row.sha256}, ${row.size}, ${row.mime})`);
	}
	if (page !== undefined) {
	...
	return result(ctx, tx, pageId, number);
};
