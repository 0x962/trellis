...
import { create, edit, list, remove, reply, resolve } from "./comments.ts";
...
describe("Page comments", () => {
	test("binds element and text threads to exact Page versions", async () => {
		...
		expect((await inTx((tx) => list(contextOf(null), tx, { page: pageId }))).map((thread) => thread.id)).toEqual([
			first.id,
			second.id,
		]);
	...
	test("lets agents reply and resolve while preserving old-version threads", async () => {
		...
		const thread = (await inTx((tx) => list(contextOf(null), tx, { page: pageId }))).find(
			(candidate) => candidate.id === threadId,
		)!;
		const replied = await inTx((tx) => reply(contextOf(agent), tx, { thread: thread!.id, body: "Done." }));
		expect(replied.comments.map((comment) => comment.actor.kind)).toEqual(["human", "agent"]);
		const resolved = await inTx((tx) => resolve(contextOf(agent), tx, { thread: thread!.id, resolved: true }));
		expect(resolved.resolved?.actor).toMatchObject({ kind: "agent", name: agentId, displayName: "Page agent" });
		expect(await inTx((tx) => list(contextOf(null), tx, { page: pageId, version: 1, resolved: true }))).toHaveLength(1);
		expect(await inTx((tx) => list(contextOf(null), tx, { page: pageId, version: 1, resolved: false }))).toHaveLength(
			0,
		);
		expect(
			(await inTx((tx) => resolve(contextOf(human), tx, { thread: thread!.id, resolved: false }))).resolved,
		).toBeNull();
	});
	...
	test("limits edit and delete to the comment author", async () => {
		...
		await expect(
			inTx((tx) => edit(contextOf(other), tx, { id: comment.id, body: "Foreign edit" })),
		).rejects.toMatchObject({ code: "INPUT_VALIDATION_FAILED" });
		await expect(inTx((tx) => remove(contextOf(other), tx, { id: comment.id }))).rejects.toMatchObject({
			code: "INPUT_VALIDATION_FAILED",
		});
		const edited = await inTx((tx) => edit(contextOf(human), tx, { id: comment.id, body: "Saved body" }));
		expect(edited.comments[0]!.body).toBe("Saved body");
		expect(await inTx((tx) => remove(contextOf(human), tx, { id: comment.id }))).toEqual({ deleted: comment.id });
		const saved = (await inTx((tx) => list(contextOf(null), tx, { page: pageId }))).find(
			(candidate) => candidate.id === thread.id,
		)!;
		expect(saved.comments[0]).toMatchObject({ body: "Comment deleted", deletedAt: at.toISOString() });
		await expect(inTx((tx) => edit(contextOf(human), tx, { id: comment.id, body: "Late edit" }))).rejects.toMatchObject(
			{
				code: "NOT_FOUND",
			},
		);
	...
	test("keeps a thread when its first comment is deleted", async () => {
		...
		await inTx((tx) => reply(contextOf(agent), tx, { thread: thread.id, body: "Reply" }));
		await inTx((tx) => remove(contextOf(human), tx, { id: thread.comments[0]!.id }));
		const saved = (await inTx((tx) => list(contextOf(null), tx, { page: pageId }))).find(
			(candidate) => candidate.id === thread.id,
		)!;
	...
	test("refuses mutations for archived projects and deleted Pages", async () => {
		...
		await expect(inTx((tx) => list(contextOf(null), tx, { page: deletedPageId }))).rejects.toMatchObject({
			code: "PAGE_DELETED",
		});
	});
});
