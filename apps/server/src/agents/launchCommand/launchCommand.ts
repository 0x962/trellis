...
export const launchCommand = (input: { run: AgentRun; url: string; context: string; directory?: string }) => {
	...
	return {
		prompt,
		command: `${input.directory ? `cd ${quote(input.directory)} && ` : ""}exec env TRELLIS_URL=${quote(url)} TRELLIS_ACTOR=${quote(actor)} claude -n ${quote(run.name)} ${quote(prompt)}`,
	};
};
