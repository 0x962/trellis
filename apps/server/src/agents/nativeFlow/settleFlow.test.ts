...
// The summary step is claimed at 2000 and its process starts at 17_000.
const running = () => {
	...
	return advanceFlow(doc, started, { type: "launched", key: `${summaryKey}:step:1`, at: 17_000 }, 17_100);
};

test("a failed child fails its box, then the run, and cancels the steps after the box", () => {
	const state = advanceFlow(doc, running(), { type: "fail", key: `${summaryKey}:step:1`, error: "It broke" }, 30_000);
	expect(state.status).toBe("failed");
	expect(state.error).toBe("It broke");
	expect(step(state, "root/1/box")).toMatchObject({ state: "failed", error: "It broke", endedAt: 30_000 });
	expect(step(state, summaryKey)).toMatchObject({ state: "failed", endedAt: 30_000 });
	expect(step(state, "root/1/checks")).toMatchObject({ state: "canceled", startedAt: null, endedAt: 30_000 });
});

test("a box past its time limit fails with its limit in minutes", () => {
	expect(step(running(), "root/1/box").deadlineAt).toBe(17_000 + 120_000);
	expect(step(advanceFlow(doc, running(), { type: "tick" }, 17_000 + 119_000), "root/1/box").state).toBe("running");
	const state = advanceFlow(doc, running(), { type: "tick" }, 17_000 + 120_000);
	expect(state.status).toBe("failed");
...
test("a finished step keeps the time it became final", () => {
	const state = advanceFlow(doc, running(), { type: "complete", key: `${summaryKey}:step:1`, output: "Done" }, 40_000);
	expect(state.status).toBe("running");
	expect(step(state, summaryKey)).toMatchObject({ state: "succeeded", startedAt: 17_000, endedAt: 40_000 });
	expect(step(state, "root/1/box")).toMatchObject({ state: "succeeded", endedAt: 40_000, output: "Done" });
