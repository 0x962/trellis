...
test("handoff backs up original settings and preserves unresolved external runs while it pauses automation", async () => {
	...
	try {
		...
		const project = await seedRoot(initial.db, "KEEP", {
			manager_config: { ade: "superset", directory: "/tmp/retained", dispatchPaused: false },
		});
		...
		try {
			...
			expect(
				(await live.db.execute(sql`SELECT manager_config FROM projects`)).rows[0]!.manager_config,
			).not.toHaveProperty("trustedDirectory");
