...
const addRun = (
	...
	h.rows(sql`INSERT INTO agent_runs
		...
		VALUES (${id},${id},${runtime},${id},${kind},'',${project},'CAP',${closed ? new Date() : null},now(),now())`);
...
test("capacity matches the scheduler rule", async () => {
	...
	expect(await h.read((tx) => capacityOf(tx, { projectId: project }))).toEqual({ used: 2, limit: 4 });
	expect(await h.read((tx) => capacityAvailable(tx, { projectId: project }))).toBe(true);

	await addRun("third", "builder", "native", false);
	await addRun("fourth", "reviewer", "native", false);
	expect(await h.read((tx) => capacityOf(tx, { projectId: project }))).toEqual({ used: 4, limit: 4 });
	expect(await h.read((tx) => capacityAvailable(tx, { projectId: project }))).toBe(false);
});
