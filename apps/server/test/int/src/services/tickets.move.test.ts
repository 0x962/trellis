...
describe("tickets.move", () => {
	...
	test("INVALID_ANCHOR for an anchor in another column", async () => {
		...
		await expectErrorData(move({ ticket: x, status: "in-progress", after: inTodo }), "INVALID_ANCHOR");
	...
	test("a reorder bumps the version but not updated_at", async () => {
		...
		expect(millis(row.updated_at)).toBe(updatedAt.getTime());
		const rows = await activityOf(h.db, y);
	...
	test("a move across columns writes the status row and the position row in one batch", async () => {
		...
		const rows = await activityOf(h.db, x);
		expect(rows.map((row) => row.field).sort()).toEqual(["position", "status"]);
		expect(rows[0]!.batch_id).toBe(rows[1]!.batch_id);
		expect(millis((await ticketRow(h.db, x))!.updated_at)).toBeGreaterThan(updatedAt.getTime());
	});
	...
	test("a move sets started_at and completed_at by category", async () => {
		...
		expect(millis(row.completed_at)).toBeGreaterThanOrEqual(millis(row.started_at));
	});
	...
	test("a move with a stale expectedVersion throws VERSION_CONFLICT", async () => {
		...
		const data = await expectErrorData(
			move({ ticket: x, status: "in-progress", expectedVersion: 2 }),
			"VERSION_CONFLICT",
		);
	...
	test("a move in an archived project throws PROJECT_ARCHIVED", async () => {
		...
		await expectErrorData(move({ ticket: x, status: "in-progress" }), "PROJECT_ARCHIVED");
