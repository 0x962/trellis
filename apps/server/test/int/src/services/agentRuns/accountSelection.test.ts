...
test("an omitted model uses the column model", async () => {
	await h.rows(
		sql`UPDATE statuses SET agent_config=jsonb_set(agent_config,'{harness}','{"preset":"claude","model":"anthropic/claude-opus-5"}') WHERE id=(SELECT status_id FROM tickets WHERE id=${ticket})`,
	);
...
test("custom commands reject a model override before assignment", async () => {
	...
	await h.rows(
		sql`UPDATE statuses SET agent_config=jsonb_set(agent_config,'{harness}','{"preset":"custom","startCommand":"echo {{prompt}}","resumeCommand":"echo {{resumeText}}"}') WHERE id=(SELECT status_id FROM tickets WHERE id=${ticket})`,
	);
	await expect(
...
test("canonical column models apply and incompatible overrides reserve nothing", async () => {
	await h.rows(
		sql`UPDATE statuses SET agent_config=jsonb_set(agent_config,'{harness}','{"preset":"claude","model":"anthropic/claude-sonnet-4.6"}') WHERE id=(SELECT status_id FROM tickets WHERE id=${ticket})`,
	);
	await expect(
