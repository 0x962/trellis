...
beforeEach(async () => {
	...
	await h.read(async (tx) => {
		...
		await tx.execute(
			sql`UPDATE projects SET manager_config=${JSON.stringify({ personaId: null, concurrency: 3, directory: fixture.home, trustedDirectory: true, harness: { preset: "claude" } })}::jsonb WHERE id=${project}`,
		);
	});
	await h.rebuild();
});
