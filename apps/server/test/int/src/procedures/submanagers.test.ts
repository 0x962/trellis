...
test("delegations round-trip through REST and RPC while human agent lists stay quiet", async () => {
	...
	await t.editServerTx(async (tx) => {
		...
		await tx.execute(sql`INSERT INTO manager_delegations (run_id,parent_run_id,project_id,brief,created_at)
			VALUES (${runId},${parentId},${child.id},'Finish child work.',now())`);
	...
	expect((await t.client.submanagers.list({}))[0]?.brief).toBe("Finish child work.");
...
test("only an active manager can start a delegation", async () => {
	const response = await t.api("/api/submanagers", {
		method: "POST",
		body: { project: "APP/child", brief: "Own this scope.", requestId: "request" },
	});
