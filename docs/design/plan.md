...
Actors: human = filled square with initials; agent = square outline in the agent color with `⟡` and a mono chip `⟡ claude-code · agent`; live dot when the agent acted within 5 min; agent comments carry an agent-colored left border; `system` never shows as an actor. Clicking an actor filters the list.
...
- Space 4 px base; all control and surface radii are 0 px, including avatars, badges, and switch thumbs (Navid, 2026-09-10).
