...
| tickets | id PK, project_id, root_id, number (CHECK > 0), title (CHECK trimmed 1..500), description ('' md), priority (CHECK set, DEFAULT none), status_id (FK statuses RESTRICT), parent_id, position double, version int DEFAULT 1, started_at, completed_at, search tsvector GENERATED (title A, description B, `english`) STORED, created_at, updated_at. UNIQUE (root_id, number). UNIQUE (id, root_id). FK (project_id, root_id) → projects (id, root_id) RESTRICT. FK (parent_id, root_id) → tickets (id, root_id) RESTRICT. CHECK `parent_id <> id`. Indexes: (project_id, status_id, position), (parent_id), partial (root_id, updated_at DESC) WHERE completed_at IS NULL, partial (root_id, completed_at DESC) WHERE completed_at IS NOT NULL, GIN (search), GIN (title gin_trgm_ops). |
...
- Backup: worker pauses its queue, `CHECKPOINT`, main spawns `tar -cf` over the data dir (about 1 s at 50k), worker resumes, gzip afterwards. Export streams NDJSON per table in 1000-row keyset pages.
...
`KEY-n` regex short-circuit; then FTS `websearch_to_tsquery('english', q) || to_tsquery('english', last || ':*')` over `tickets.search` and `comments.search` (grouped by ticket) ranked by `ts_rank`; unioned with `title <% q` ranked by `word_similarity` when `q.length >= 3`; deduped, limit 20, 200 ms statement timeout. Client: 120 ms debounce, one in-flight search, superseded ones dropped on both sides.
