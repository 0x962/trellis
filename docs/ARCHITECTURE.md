...
A failed login shell returns a tool error. The next caller runs the login shell again. The gh poller is one such caller. While gh is not ok, the poller runs `gh auth status` once per minute. Each run starts a new login shell. Existing agent controls use their saved launch environment.
