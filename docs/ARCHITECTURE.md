...
A manual refresh has a ten-second minimum interval. OpenCode and Pi return `unsupported` quota. Muse returns `signed_out` for a profile without `muse/auth.json` and `unsupported` otherwise, because Muse reports its windows only inside a running session host.
...
`usage.report` reads the transcript files that each harness CLI writes: `projects/` of a Claude profile, `sessions/` of a Codex or Pi profile, `opencode/storage/` of an OpenCode data home, and `muse/sessions/` of a Muse data home.
