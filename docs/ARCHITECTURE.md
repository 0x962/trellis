...
A failed login shell returns a tool error. The next caller runs the login shell again. The gh poller is one such caller: while gh is not ok, it runs `gh auth status` once per minute, and each run starts a new login shell. Existing agent controls use their saved launch environment.
