...
A `turn/start` during a running turn, even with `ifBusy: queue`, enters the Muse inbox as a steer item. Muse drains it at a point of its own choice, and on 2026-09-16 at 22:52:46 that point was the middle of a model step, which Muse cancelled: the running turn ended as interrupted and its work moved to the next turn. The bridge therefore holds every prompt that arrives during a turn and starts one turn with all of them when the turn ends.
