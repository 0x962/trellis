...
To pair the phone, open Settings, then Server. Type the server URL and tap Test connection. The app shows the server version and the ticket count. Type your name and tap Save.

The server listens on `127.0.0.1` only. A phone reaches it through a forward that you set up, for example an SSH tunnel or a Tailscale proxy to port 4521.
