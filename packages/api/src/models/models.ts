...
// The canonical id of a model that a harness program listed under its own
// name, or undefined when this build cannot launch that model. A harness
// account offers whatever its provider sells, and `catalog.json` holds the
// models trellis knows. `fromHarnessModel` throws on a name it cannot place,
// and `supportsModel` rejects a model the harness cannot run.
export function catalogModelFor(harness: Exclude<Harness, "custom">, name: string): string | undefined {
	let id: string;
	try {
		id = fromHarnessModel(harness, name);
	} catch {
		return undefined;
	}
	return supportsModel(harness, id) ? id : undefined;
}
