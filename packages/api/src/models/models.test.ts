...
test("a listed harness model reaches the catalog only when this build can launch it", () => {
	expect(catalogModelFor("claude", "sonnet")).toBe("anthropic/claude-sonnet-5");
	expect(catalogModelFor("codex", "gpt-5.6-sol")).toBe("openai/gpt-5.6-sol");
	expect(catalogModelFor("muse", "muse-spark-1.3")).toBe("meta/muse-spark-1.3");
	expect(catalogModelFor("pi", "vercel-ai-gateway/anthropic/claude-fable-5")).toBe("anthropic/claude-fable-5");
	// The catalog holds no such model, so the picker cannot offer it.
	expect(catalogModelFor("claude", "opus[1m]")).toBeUndefined();
	expect(catalogModelFor("opencode", "opencode/big-pickle")).toBeUndefined();
	// codex runs no Llama model, so this name maps to a model it cannot run.
