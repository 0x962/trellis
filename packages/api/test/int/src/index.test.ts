...
test("the index re-exports every public symbol with no side effect on import", async () => {
	const child = Bun.spawnSync({
		cmd: [process.execPath, join(originDir(import.meta.dir), "..", "test", "importSideEffects.ts")],
		stdout: "pipe",
		stderr: "pipe",
	});
...
test("every value import under src/ names a declared dependency", async () => {
	...
	for (const file of new Bun.Glob("**/*.ts").scanSync({ cwd: originDir(import.meta.dir) })) {
		if (file.endsWith(".test.ts")) continue;
		const source = await Bun.file(join(originDir(import.meta.dir), file)).text();
