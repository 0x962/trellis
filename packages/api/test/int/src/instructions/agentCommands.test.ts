...
import { agentLaunch, restartText, resumeCommand } from "../../../../src/instructions/agentLaunch.ts";
import {
	agentActor,
	agentTitle,
	builderBranch,
	...
	projectTag,
} from "../../../../src/instructions/agentNames.ts";
...
const bin = mkdtempSync(join(tmpdir(), "trellis-commands-"));
...
const launch = (input: Parameters<typeof agentLaunch>[0]) => agentLaunch(input).command;

describe("agent names", () => {
	test("each role acts as its own actor, from the project path or the ticket identifier", () => {
		expect(agentActor({ role: "manager", project: "CDE" })).toBe("agent:manager-cde");
		expect(agentActor({ role: "manager", project: "CDE.web" })).toBe("agent:manager-cde.web");
		expect(agentActor({ role: "builder", ticket: "CDE-42" })).toBe("agent:builder-cde-42");
		expect(agentActor({ role: "reviewer", ticket: "CDE-42" })).toBe("agent:reviewer-cde-42");
	});

	test("tabs, workspaces, tags, and branches carry the project and the ticket", () => {
		expect(agentTitle({ role: "manager", project: "CDE" })).toBe("CDE manager");
		expect(agentTitle({ role: "reviewer", ticket: "CDE-42" })).toBe("CDE-42 review");
...
describe("launch commands", () => {
	test("a builder runs claude named after its ticket, as its own actor, with the builder prompt", () => {
		const { claude, trellis } = run(launch({ role: "builder", project: "CDE", ticket: "CDE-42", url }));
	...
	test("a reviewer tab is named '<ticket> review' and its prompt names the PR", () => {
		...
		const { claude, trellis } = run(launch({ role: "reviewer", project: "CDE", ticket: "CDE-42", prUrl, url }));
	...
	test("the manager tab is named '<project> manager' and acts as the project's manager", () => {
		const { claude, trellis } = run(launch({ role: "manager", project: "CDE.web", url }));
	...
	test("every command is one line, so a terminal that types it runs it whole", () => {
		for (const command of [
			launch({ role: "manager", project: "CDE", url }),
			launch({ role: "builder", project: "CDE", ticket: "CDE-42", url }),
			launch({ role: "reviewer", project: "CDE", ticket: "CDE-42", prUrl: "https://x/pull/1", url }),
