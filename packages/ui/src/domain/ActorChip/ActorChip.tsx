...
export type ActorChipProps = {
	...
	live?: boolean;
	// Drops the "· agent" suffix, for a narrow row. The purple name
...
// An agent name is purple with an "· agent" suffix, so a human and
...
export function ActorChip({ name, kind, live = false, compact = false, className }: ActorChipProps) {
	return (
		<span className={cx("inline-flex items-center gap-1.5 whitespace-nowrap", className)}>
			...
			<span className={kind === "agent" ? "text-sm text-agent" : "font-medium text-fg"}>{name}</span>
