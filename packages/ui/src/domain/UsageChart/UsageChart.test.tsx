...
const series = [
	...
	{ key: "codex", label: "Codex", tone: "accent" as const, values: [0, 1, 1] },
];
const format = (value: number) => `$${value.toFixed(2)}`;
const formatDay = (day: string) => day.slice(5);

describe("UsageChart", () => {
	test("draws one layered area per series and one button per day", () => {
		...
		expect(container.querySelector("[data-series='claude']")?.classList.contains("text-agent")).toBe(true);
		...
		// The top gridline is a round figure at or above the largest value.
