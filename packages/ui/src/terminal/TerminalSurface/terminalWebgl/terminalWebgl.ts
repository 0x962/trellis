		...
		try {
			...
			listeners.push(
				addon.onAddTextureAtlasCanvas(() => {
					...
					atlasPages = 0;
					// Each color variant consumes texture space. Release accumulated pages after the current glyph draw completes.
					queueMicrotask(() => addon?.clearTextureAtlas());
				}),
			);
			terminal.loadAddon(addon);
		} catch {
			// A browser can refuse a GPU context. The terminal's DOM renderer remains available after the addon releases its resources.
			release();
			terminal.refresh(0, terminal.rows - 1);
		}
	});
