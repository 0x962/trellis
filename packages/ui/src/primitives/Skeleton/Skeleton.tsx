...
// A placeholder for text that has not arrived. Assistive tech skips it; the
// surrounding region announces the loading state through aria-busy.
	...
	return (
		<div aria-busy="true" aria-hidden="true" className={cx("flex flex-col gap-2", className)}>
			{[...Array(lines).keys()].map((line) => (
				<div
					key={line}
					className={cx("rounded-sm bg-border animate-pulse-live motion-reduce:animate-none", width, height)}
				/>
			))}
		</div>
	);
