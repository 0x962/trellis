...
// One flow run of the ticket that owns the pull request, at any commit.
type Run = { slug: string; name: string; status: string };
...
export const flowReadiness = async (
	...
	const [records, waiver] = await Promise.all([
		client.flowExecutions.list({ ticket }),
...
// An agent states the reason once, and `trellis ready` takes it from then on.
