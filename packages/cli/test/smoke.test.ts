...
// A test here starts servers and CLI processes. While the parallel tasks of
// `bun run check` load the machine, one test can take longer than the 5 s
// default of bun test.
...
describe("the live CLI", () => {
	test(
		"the smoke chain passes against the real server",
		async () => {
	...
	test(
		"the policy and connection failures use the specified exit codes",
		async () => {
	...
	test(
		"serve runs the server until SIGTERM",
		async () => {
	...
	test(
		"backup and restore reproduce the ticket list",
		async () => {
	...
	test(
		"restore refuses while the server answers",
		async () => {
	...
	test(
		"status prints the live health fields",
		async () => {
...
describe("local installation files", () => {
	...
	test(
		"uninstall removes the installed files and leaves the data home",
		async () => {
	...
	test(
		"logs prints the last lines and follows appended lines",
		async () => {
